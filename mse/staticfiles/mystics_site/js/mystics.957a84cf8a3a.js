(() => {
  ...
  if (toggle) {
    toggle.addEventListener("click", () => {
      ...
      // Light re-init (charts are theme-aware via tick/grid colors)
      initChartsSafe();
    });
  }
  ...
  function playerFallback() {
    // Support both names (older templates + new templates)
    return (
      (ms().staticPlayerFallback || "") ||
      (window.MS_STATIC_PLAYER_FALLBACK || "") ||
      ""
    );
  }
  ...
  function makeLineChart(canvas, labels, series) {
    const { grid, tick } = chartColors();

    // Chart.js may not be present on every page — fail safely.
  ...
  const charts = { teamTrend: null };

  async function initTeamTrend() {
    const canvas = document.getElementById("teamTrend");
    if (!canvas) return;

    // Prefer mysticsTrendUrl if provided, else teamTrendUrl
    ...
    try {
      ...
      if (!labels.length || !pts.length) {
        // No data — keep page clean
        return;
      }

      charts.teamTrend?.destroy?.();
      charts.teamTrend = makeLineChart(canvas, labels, pts);
    } catch (e) {
  ...
  /* -----------------------------
   * Roster table (prevents /undefined)
   * ----------------------------- */
  async function initRosterTable() {
    ...
    try {
      ...
      for (const p of results) {
        ...
        if (!id) continue; // ✅ never generate /undefined
  ...
  document.addEventListener("DOMContentLoaded", () => {
    // Attach fallbacks (supports templates that use data-fallback too)
